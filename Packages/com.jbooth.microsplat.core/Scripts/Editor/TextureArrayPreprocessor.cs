   ...
   class TextureArrayPreProcessor : AssetPostprocessor
   {
      static int GetNewHash(TextureArrayConfig cfg)
      {
         unchecked
         {
            ...
            h = h * TextureArrayConfigEditor.GetTextureFormat(cfg, settings.diffuseSettings.compression, settings.diffuseSettings.compressionQuality).GetHashCode() * 7;
            h = h * TextureArrayConfigEditor.GetTextureFormat(cfg, settings.normalSettings.compression, settings.normalSettings.compressionQuality).GetHashCode() * 13;
            h = h * TextureArrayConfigEditor.GetTextureFormat(cfg, settings.emissiveSettings.compression, settings.emissiveSettings.compressionQuality).GetHashCode() * 17;
            h = h * TextureArrayConfigEditor.GetTextureFormat(cfg, settings.antiTileSettings.compression, settings.antiTileSettings.compressionQuality).GetHashCode() * 31;
            h = h * TextureArrayConfigEditor.GetTextureFormat(cfg, settings.smoothSettings.compression, settings.smoothSettings.compressionQuality).GetHashCode() * 37;
